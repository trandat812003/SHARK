...
class SinusoidalPositionalEmbedding(nn.Embedding):
    ...
    def __init__(self, num_positions, embedding_dim, padding_idx=None):
        ...
        self.weight = nn.Parameter(self._init_weight(), requires_grad=False)
    ...
    def _init_weight():
        ...
        """
        n_pos, dim = 1026, 768  # You should adjust these values based on your specific use case
        position_enc = np.array(
            [[pos / np.power(10000, 2 * (j // 2) / dim) for j in range(dim)] for pos in range(n_pos)]
        )

        sin_tensor = torch.FloatTensor(np.sin(position_enc[:, 0::2]))
        cos_tensor = torch.FloatTensor(np.cos(position_enc[:, 1::2]))

        out_new = torch.zeros(n_pos, dim)
        out_new[:, 0:dim // 2] = sin_tensor
        out_new[:, dim // 2:] = cos_tensor
        return out_new
