...
from fastNLP.models.torch.seq2seq_generator import Seq2SeqModel
from fastNLP.modules.torch.decoder.seq2seq_decoder import Seq2SeqDecoder
...
class TransformerUnit(nn.Module):
    ...
    def forward(self, features):
        features = self.transformerlayer(features)
        return features
...
class SequenceGeneratorModel(nn.Module):
    def __init__(
        ...
        self.generator = SequenceGenerator(seq2seq_model.decoder, max_length=max_length, max_len_a=max_len_a, num_beams=num_beams, do_sample=do_sample,
        ...
        if use_gate:
            ...
            self.linear_layer1 = nn.Linear(self.hidden_size * 2, self.hidden_size)

        self.fuse_type = fuse_type
        self.use_gate = use_gate
        self.use_retrieval_CSK = use_retrieval_CSK
        self.use_generated_CSK = use_generated_CSK
...
class SequenceGenerator:
    def __init__(self, decoder: Seq2SeqDecoder, max_length=20, max_len_a=0.0, num_beams=1,
        ...
        self.generate_func = partial(greedy_generate, decoder=decoder, max_length=max_length, max_len_a=max_len_a,
                                     ...
                                     restricter=restricter) # 偏函数：输入扩展函数及固定的参数
    ...
    @torch.no_grad()
    def generate(self, state, utt_prefix_ids, dia_utt_num, tokens=None):
        """
        ...
        return self.generate_func(utt_prefix_ids=utt_prefix_ids, dia_utt_num=dia_utt_num, tokens=tokens, state=state)


@torch.no_grad()
def greedy_generate(decoder, utt_prefix_ids=None, dia_utt_num=None, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=1,
    ...
    """
    if num_beams == 1:
        token_ids = _no_beam_search_generate(decoder, utt_prefix_ids=utt_prefix_ids, dia_utt_num=dia_utt_num, tokens=tokens, state=state, max_length=max_length, max_len_a=max_len_a,
                                             bos_token_id=bos_token_id, eos_token_id=eos_token_id,
                                             repetition_penalty=repetition_penalty, length_penalty=length_penalty,
                                             pad_token_id=pad_token_id, restricter=restricter)
    else:
        token_ids = _beam_search_generate(decoder, utt_prefix_ids=utt_prefix_ids, dia_utt_num=dia_utt_num, tokens=tokens, state=state, max_length=max_length, max_len_a=max_len_a,
                                          num_beams=num_beams,
...
def _no_beam_search_generate(decoder: Seq2SeqDecoder, utt_prefix_ids=None, dia_utt_num=None, state=None, tokens=None, max_length=20, max_len_a=0.0, bos_token_id=None,
                             eos_token_id=None,
    ...
    scores = decoder.decode(tokens, utt_prefix_ids, dia_utt_num, state)  # 主要是为了update state
    ...
    while cur_len < real_max_length:
        scores = decoder.decode(token_ids, utt_prefix_ids, dia_utt_num, state)  # batch_size x vocab_size
...
def _beam_search_generate(decoder: Seq2SeqDecoder, utt_prefix_ids=None, dia_utt_num=None, tokens=None, state=None, max_length=20, max_len_a=0.0, num_beams=4,
    ...
    scores = decoder.decode(tokens, utt_prefix_ids, dia_utt_num, state)  # 这里要传入的是整个句子的长度
    # 这里需要考虑如果在第一个位置就结束的情况
    # if _eos_token_id!=-1:
    #     scores[:, _eos_token_id] = -1e12
    ...
    while cur_len < real_max_length:
        scores = decoder.decode(token_ids, utt_prefix_ids, dia_utt_num, state)  # (bsz x num_beams, vocab_size)
        if repetition_penalty != 1.0:
            token_scores = scores.gather(dim=1, index=token_ids)
            lt_zero_mask = token_scores.lt(0).float()
            ge_zero_mask = lt_zero_mask.eq(0).float()
